...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
                                      DataBox value) {
    List<Bucket<Integer>> l = this.getAllBuckets();
    Integer min = l.get(0).getStart();
    Iterator<Bucket<Integer>> iter = l.iterator();

    Integer max = this.getMaxValue();
    while (iter.hasNext()) {
      Bucket<Integer> b = iter.next();
      if (b.getCount() > 0) {
        max = b.getEnd();
      }
    }
    if (predicate == PredicateOperator.EQUALS) {
      return (float) 1.0/ (float) this.getNumDistinct();
    } else if (predicate == PredicateOperator.LESS_THAN_EQUALS) {
      return 1.0f - (float) (max - (value.getInt()+1))/(float) (max - this.getMinValue());
    } else if (predicate == PredicateOperator.LESS_THAN) {
      return 1.0f -  (float)(max - (value.getInt()))/(float)(max - this.getMinValue());
    } else if (predicate == PredicateOperator.GREATER_THAN_EQUALS) {
      return (float) (max - (value.getInt()-1))/(float) (max - this.getMinValue());

    } else if (predicate == PredicateOperator.GREATER_THAN) {

      return (float) (max - value.getInt())/ (float)(max - this.getMinValue());
    } else {
      return 0.1f;
    }
  }
