...
public class IndexScanOperator extends QueryOperator {
  ...
  public int estimateIOCost() throws QueryPlanException {
    try {
      float rf = this.transaction.getStats(tableName).getReductionFactor(this.columnIndex, this.predicate, this.value);
      long rec = this.transaction.getNumRecords(tableName);
      int p2 = this.transaction.getNumIndexPages(tableName, this.columnName);
      return (int) Math.ceil(rf * (rec + p2));
